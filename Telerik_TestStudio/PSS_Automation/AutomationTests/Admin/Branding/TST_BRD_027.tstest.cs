...
namespace PSS_Automation
{
    ...
    public class TST_BRD_027 : BaseWebAiiTest
    {
        ...
        public void TST_BRD_027_CodedStep()
        {if(Pages.PS_BrandingLookAndFeelPage.PrintLogoImg.IsVisible()){ 
             string prevImg = Pages.PS_BrandingLookAndFeelPage.PrintLogoImg.BaseElement.GetAttribute("src").Value.ToString();             
             ...
             string currImg = Pages.PS_BrandingLookAndFeelPage.PrintLogoImg.BaseElement.GetAttribute("src").Value.ToString();             
             string preImage = GetExtractedValue("PrevImg").ToString();
             if(preImage.Length > 0){
             Assert.IsTrue(!preImage.Equals(currImg), "print logo should be changed after upload...");  
             }
             else{
                 Assert.IsTrue(Pages.PS_BrandingLookAndFeelPage.PrintLogoImg.IsVisible(),"print logo should be visible");
             }             
        }
    }
}
