...
namespace PSS_Automation
{
    ...
    public class TST_BRD_037 : BaseWebAiiTest
    {
        ...
        public void TST_BRD_037_CodedStep()
        {
             if(Pages.PS_BrandingLookAndFeelPage.LoginLogoImg.IsVisible()){ 
             string prevImg = Pages.PS_BrandingLookAndFeelPage.LoginLogoImg.BaseElement.GetAttribute("src").Value.ToString();             
             ...
             string currImg = Pages.PS_BrandingLookAndFeelPage.LoginLogoImg.BaseElement.GetAttribute("src").Value.ToString();
             string preImage = GetExtractedValue("PrevImg").ToString();
             if(preImage.Length > 0){
             Assert.IsTrue(!preImage.Equals(currImg), "login logo should be changed after upload...");  
             }
             else{
                 Assert.IsTrue(Pages.PS_BrandingLookAndFeelPage.LoginLogoImg.IsVisible(),"login logo should be visible");
             }              
        }
    
        [CodedStep(@"New Coded Step")]
        public void TST_BRD_037_CodedStep1()
        {
