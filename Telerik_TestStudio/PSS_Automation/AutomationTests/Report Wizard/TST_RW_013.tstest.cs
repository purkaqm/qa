...
namespace PSS_Automation
{
    ...
    public class TST_RW_013 : BaseWebAiiTest
    {
        ...
        [CodedStep(@"Select 'Rejected On' checkbox")]
        public void TST_RW_013_CS24()
        {
            ActiveBrowser.RefreshDomTree();
            HtmlInputCheckBox rejectedOnChkbox = ActiveBrowser.Find.ByXPath<HtmlInputCheckBox>(AppLocators.get("html_report_rejected_on_chkbox"));
            rejectedOnChkbox.Wait.ForExists();
            rejectedOnChkbox.Click();
        }
        ...
        public void TST_RW_013_CS35()
        {
            Assert.IsTrue(ActiveBrowser.Find.AllByXPath(string.Format(AppLocators.get("output_result_report_name"),reportName)).Count > 0);
                                                
        }
    
        [CodedStep(@"Verify 'Rejected on' column is present")]
        public void TST_RW_013_CS36()
        {
            Pages.PS_HtmlReportPage.RejectedOnSpan.Wait.ForExists();
            Assert.IsTrue(Pages.PS_HtmlReportPage.RejectedOnSpan.IsVisible(),"Nominated on column should be present in html report page");
        }
    
        [CodedStep(@"Verify project with 'Rejected on' date is present")]
        public void TST_RW_013_CS37()
        {
            ...
            HtmlSpan rejectedOnDate = ActiveBrowser.Find.ByXPath<HtmlSpan>(string.Format(AppLocators.get("html_report_rejected_on_date"),GetExtractedValue("CreatedProjectName").ToString(),reportDate));
            rejectedOnDate.Wait.ForVisible(); 
            Assert.IsTrue(rejectedOnDate.IsVisible(),"Correct date should be display in 'Nominated on' column"); 
        }
    
        [CodedStep("Close report html page")]
        public void TST_RW_013_CS38()
        {
            Manager.ActiveBrowser.Close();                                    
        }
    
        [CodedStep(@"Delete created report")]
        public void TST_RW_013_CS39()
        {
            try
            {
