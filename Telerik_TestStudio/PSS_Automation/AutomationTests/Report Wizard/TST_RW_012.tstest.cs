...
namespace PSS_Automation
{
    ...
    public class TST_RW_012 : BaseWebAiiTest
    {
        ...
        string reportName,nominationCmt,rejectedComment;
    
        [CodedStep(@"Wait for user to be navigated to newly created work summary page")]
        public void TST_RW_012_CS01()
        {
        ...
        public void TST_RW_012_CS09()
        {
            rejectedComment = Data["RejectedComment"].ToString();
            Actions.SetText(Pages.PS_ProjectSummaryPage.CommentTextArea,rejectedComment);
        ...
        [CodedStep(@"Select 'Rejected Comment' checkbox")]
        public void TST_RW_012_CS24()
        {
            ActiveBrowser.RefreshDomTree();
            HtmlInputCheckBox rejectedCmtChkbox = ActiveBrowser.Find.ByXPath<HtmlInputCheckBox>(AppLocators.get("html_report_rejection_cmt_chkbox"));
            rejectedCmtChkbox.Wait.ForExists();
            rejectedCmtChkbox.Click();
        }
    
        [CodedStep(@"Select 'Any of' option")]
        public void TST_RW_012_CS25()
        {
        ...
        [CodedStep(@"Set text in Rejected Comment field in 'Filter' part of report")]
        public void TST_RW_012_CS26()
        {
            rejectedComment = Data["RejectedComment"].ToString();
            Actions.SetText(Pages.PS_AddReportWizardPage.FilterStartText,rejectedComment);
        }
    
        [CodedStep(@"Click on Save button")]
        public void TST_RW_012_CS27()
        {
        ...
        public void TST_RW_012_CS35()
        {
            Assert.IsTrue(ActiveBrowser.Find.AllByXPath(string.Format(AppLocators.get("output_result_report_name"),reportName)).Count > 0);
                                    
        }
    
        [CodedStep(@"Verify 'Rejected Comment' column is present")]
        public void TST_RW_012_CS36()
        {
            Pages.PS_HtmlReportPage.RejectedCmtSpan.Wait.ForExists();
            Assert.IsTrue(Pages.PS_HtmlReportPage.RejectedCmtSpan.IsVisible(),"Rejected comment column should be present in html report page");
            
        }
    
        [CodedStep(@"Verify Project with 'Rejection comment' is present")]
        public void TST_RW_012_CS37()
        {
            ...
            string rejCmtValue = string.Format(AppLocators.get("html_report_rejected_comment"),GetExtractedValue("CreatedProjectName").ToString(),rejectedComment.Trim());
            Log.WriteLine(rejCmtValue);
            HtmlTableRow resRejComment = ActiveBrowser.Find.ByXPath<HtmlTableRow>(rejCmtValue);
            resRejComment.Wait.ForExists(); 
            Assert.IsTrue(resRejComment.IsVisible(),"Right comment should be present in 'Rejected comment' column"); 
        }
    
        [CodedStep(@"Close report html page")]
        public void TST_RW_012_CS38()
        {
