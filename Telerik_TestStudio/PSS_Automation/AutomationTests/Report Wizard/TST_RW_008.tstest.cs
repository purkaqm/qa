...
namespace PSS_Automation
{
    ...
    public class TST_RW_008 : BaseWebAiiTest
    {
        ...
        public void TST_RW_008_CS21()
        {
            ActiveBrowser.RefreshDomTree();
            HtmlInputCheckBox nominatedByChkbox = ActiveBrowser.Find.ByXPath<HtmlInputCheckBox>(AppLocators.get("report_wiz_nominated_by_chkbox"));
            nominatedByChkbox.Wait.ForExists();
            nominatedByChkbox.Click();
        }
    
        [CodedStep(@"Select 'Any of' option")]
        public void TST_RW_008_CS22()
        {            
            Pages.PS_AddReportWizardPage.SetProjectSelect.SelectByText("Any of");  
        }
    
        [CodedStep(@"Select 'Approved by' user")]
        public void TST_RW_008_CS23()
        {
        ...
        public void TST_RW_008_CS34()
        {
            Assert.IsTrue(ActiveBrowser.Find.AllByXPath(string.Format(AppLocators.get("output_result_report_name"),reportName)).Count > 0);
                        
        }
    
        [CodedStep(@"Verify 'Nominated by' column is present")]
        public void TST_RW_008_CS35()
        {
            Pages.PS_HtmlReportPage.NominatedBySpan.Wait.ForExists();
            Assert.IsTrue(Pages.PS_HtmlReportPage.NominatedBySpan.IsVisible(),"Approved by column should be present in html report page");
        }
    
        [CodedStep(@"Verify Project with 'Nominated By' user name is present")]
        public void TST_RW_008_CS36()
        {
            ...
            HtmlSpan resNomByName = ActiveBrowser.Find.ByXPath<HtmlSpan>(string.Format(AppLocators.get("html_report_nominated_by_user_name"),GetExtractedValue("CreatedProjectName").ToString(),userName));
            resNomByName.Wait.ForVisible(); 
            Assert.IsTrue(resNomByName.IsVisible(),"User name should be present in 'Approved by' column"); 
        }
    
        [CodedStep(@"Close the report html page")]
        public void TST_RW_008_CS37()
        {
