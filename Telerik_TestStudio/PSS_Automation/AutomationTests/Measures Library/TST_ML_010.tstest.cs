...
namespace PSS_Automation
{
    ...
    public class TST_ML_010 : BaseWebAiiTest
    {
        ...
        [CodedStep(@"[TST_ML_009_CS04] : Select 'Copy' on drop down menu near 'X_measure_template01'")]
        public void TST_ML_010_CS04()
        {
        ...
        public void TST_ML_010_CS06()
        {
            Pages.PS_MeasureTemplatesNewPage.SubmitButton.Click();
            ActiveBrowser.WaitUntilReady();
        }
        
        [CodedStep(@"Verify Copy of 'X_measure_template01' Measure library template is created")]
        public void TST_ML_010_CS07()
        {
            Pages.PS_ReviewMeasureLibrary.ShowMoreLessImage.Click();
            System.Threading.Thread.Sleep(3000);
            Pages.PS_ReviewMeasureLibrary.TitleColumnLink.Click(true);
            ...
            ActiveBrowser.WaitUntilReady();
            ActiveBrowser.RefreshDomTree();
            Assert.IsTrue(ActiveBrowser.Find.AllByXPath(string.Format(AppLocators.get("measure_library_name_link"),measureName + "-Copy")).Count>0);
        }
    
        [CodedStep(@"Delete generated Measure library")]
        public void TST_ML_010_CS08()
        {
                        HtmlImage measureNameTraingleImg = ActiveBrowser.Find.ByXPath<HtmlImage>(string.Format(AppLocators.get("measure_library_traingle_img"),measureName));
                        Log.WriteLine(string.Format(AppLocators.get("measure_library_traingle_img"),measureName));
                        measureNameTraingleImg.MouseClick();
