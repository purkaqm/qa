...
import com.powersteeringsoftware.libs.elements.StatusChooser;
 ...
 * @author karina
 */
public class ProjectsASPage extends AbstractASPage {

    @Override
    public void open() {
        openAdvancedSearch().openProjectsTab();
    }
    ...
    public void selectDescendedFrom(String descendent, boolean howToSet) {
        WorkChooserDialog dialog = new WorkChooserDialog(PROJECT_DESCENDED_FIELD, PROJECT_DESCENDED_POPUP);
        dialog.open();
        if (howToSet) {
            // set using browse
    ...
    public UserDialog getOwnerDialog() {
        // Click the Search menu
        /*Link link = new Link(OWNER_POPUP);
        link.click(false);

        return new OwnerDialog(OWNER_POPUP); */
        return new UserDialog(OWNER_FIELD, OWNER_POPUP);
    }

    public StatusChooser getStatusChooser() {
        return new StatusChooser(STATUS_CHOOSER_FIELD);
    }
